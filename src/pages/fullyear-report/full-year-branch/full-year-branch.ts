...
/**
 * Generated class for the FullYearBranchPage page.
 *
...
@IonicPage()
@Component({
  selector: 'page-full-year-branch',
  templateUrl: 'full-year-branch.html',
})
export class FullYearBranchPage {
	...
	constructor(public navCtrl: NavController, public navParams: NavParams, public service: AdapterServiceProvider,
		...
		service.analytics("Full Year Branch");
		...
		/** calling Full Year Branch service */
		this.fullYearBranch(this.agentsList[0]['agentCode']);
		}else{
	...
	setAction (item, selectedID){
		...
		this.fullYearBranch(selectedID);
		this.fullYearResult = [];
	}


	/** Getting Data from Full Year Branch SErvice */
	fullYearBranch(agentID){
		if (this.utils.isDeviceOnLine()) {
			this.fullYearService.getFullYearBranchData(agentID).then((res)=>{
				...
				try{
					...
					this.logger.log("Full Year Branch Service success response"+JSON.stringify(res));
					...
					this.logger.error("Full Year Branch Service catch error "+JSON.stringify(e));
				}
			},(err)=>{
				this.serviceCallCompleted = true;
				this.utils.hideLoader();
				...
				this.logger.error("Full Year Branch Service failure "+JSON.stringify(err));
