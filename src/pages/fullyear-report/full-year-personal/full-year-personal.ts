...
/**
 * Generated class for the FullYearPersonalPage page.
 *
...
@IonicPage()
@Component({
  selector: 'page-full-year-personal',
  templateUrl: 'full-year-personal.html',
})
export class FullYearPersonalPage {
	...
	constructor(public navCtrl: NavController, public navParams: NavParams, public service: AdapterServiceProvider,
		...
		service.analytics("Full Year Personal");
		...
		/** calling Full Year Personal service */
		this.fullYearPersonal(this.agentsList[0]['agentCode']);
		}else{
	...
	setAction (item, selectedID){
		...
		this.fullYearPersonal(selectedID);
		this.fullYearResult = [];
	}


	/** Getting Data from Full Year Personal SErvice */
	fullYearPersonal(agentID){
		if (this.utils.isDeviceOnLine()) {
			this.fullYearService.getFullYearPersonalData(agentID).then((res)=>{
				...
				try{
					...
					this.logger.log("Full Year Personal Service success response"+JSON.stringify(res));
					...
					this.logger.error("Full Year Personal Service catch error "+JSON.stringify(e));
				}
			},(err)=>{
				this.serviceCallCompleted = true;
				this.utils.hideLoader();
				...
				this.logger.error("Full Year Personal Service failure "+JSON.stringify(err));
