...
@IonicPage()
@Component({
  selector: 'page-sun-elite-branch',
  templateUrl: 'sun-elite-branch.html',
})
export class SunElitePageBranch {
	...
	constructor(public navCtrl: NavController, public navParams: NavParams,public strings: StringsProvider, private  service: AdapterServiceProvider,public utils: UtilsProvider,private logger : LoggerServiceProvider,
		public config : ConfigProvider, public sunEliteService: SunEliteServiceProvider) {
		service.analytics("SunElite Branch");
		...
		/** calling SunElite Branch service */
		this.sunEliteBranch(this.agentsList[0]['agentCode']);
		}else{
	...
	setAction (item, selectedID){
		...
		this.sunEliteBranch(selectedID);
		this.sunEliteVal = [];
	}
	...
	/** Getting Data from SunElite Branch SErvice */
	sunEliteBranch(agentID){
		if (this.utils.isDeviceOnLine()) {
			this.sunEliteService.getSunEliteBranchData(agentID).then((res)=>{
				...
				try{
					...
					this.logger.log("SunElite Branch Service success response"+JSON.stringify(res));
					...
					this.logger.error("SunElite Branch Service catch error "+JSON.stringify(e));
				}
			},(err)=>{
				this.serviceCallCompleted = true;				
				this.utils.hideLoader();
				...
				this.logger.error("SunElite Branch Service failure "+JSON.stringify(err));
